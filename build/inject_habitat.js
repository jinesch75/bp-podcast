// Append the Observatoire de l'Habitat episode (id 17, key 'habitat') to episodes_data.js.
const fs = require('fs');
...
if (EPISODES.find(function (e) { return e.key === 'habitat'; })) throw new Error('habitat already present');

const c = JSON.parse(fs.readFileSync(OUT + '/habitat_content.json', 'utf8'));
const en = JSON.parse(fs.readFileSync(TMP + '/habitat_seg/segdata_fixed.json', 'utf8'));
const fr = JSON.parse(fs.readFileSync(TMP + '/habitat_fr_seg/segdata_fixed.json', 'utf8'));
const de = JSON.parse(fs.readFileSync(TMP + '/habitat_de_seg/segdata_fixed.json', 'utf8'));
const lb = JSON.parse(fs.readFileSync(TMP + '/habitat_lb_segments.json', 'utf8'));
...
const ep = {
  ...
  audio_fr: 'podcast_habitat_fr.mp3', duration_fr: fr.duration, segments_fr: fr.segments,
  audio_de: 'podcast_habitat_de.mp3', duration_de: de.duration, segments_de: de.segments,
  segments_lb: lb
};
...
console.log('habitat:', 'dur', ep.duration, '| segEN', ep.segments.length, 'FR', ep.segments_fr.length, 'DE', ep.segments_de.length, 'LB', ep.segments_lb.length,
